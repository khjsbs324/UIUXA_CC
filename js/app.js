...
    };
    try {
        if (fc.apiKey && fc.apiKey !== "여기에_API_KEY_입력") {
...
    let pwdActionType = 'editMode'; let pwdActionPayload = null; let noticeDeleteTimer = null; let scrollDirection = 'down'; let lastScrollTopPos = 0; let scrollHideTimeout = null;
...
        if (isEditMode) {
            isEditMode = false; window.saveToFirebase(); window.renderAll(); window.updateEditModeUI(); window.showToast("저장 후 종료되었습니다."); return;
        }
...
            } else {
                noticeData.activePopupId = pwdActionPayload; window.saveToFirebase(); window.renderNoticeBoard(); window.closePwdModal(); window.showToast("팝업 등록됨");
            }
        } else {
...
                    <button onclick="event.stopPropagation(); window.clearBoardFile();" class="text-slate-400 hover:text-red-500"><i data-lucide="x" class="w-3.5 h-3.5"></i></button>
                </span>
            `;
            lucide.createIcons();
        }
...
                        </div>
                    </div>
                    <button onclick="window.downloadMockFile('${post.id}')" class="w-8 h-8 rounded-lg bg-white hover:bg-figjamLight hover:text-figjam border border-slate-100 text-slate-400 flex items-center justify-center transition-all shadow-sm">
...
                            <button onclick="window.editBoardPost('${post.id}')" class="text-slate-400 hover:text-figjam bg-slate-50 p-1.5 rounded-lg border border-slate-100" title="결과물 교체/수정"><i data-lucide="edit-3" class="w-4 h-4"></i></button>
                            <button onclick="window.deleteBoardPost('${post.id}')" class="text-slate-400 hover:text-red-500 bg-slate-50 p-1.5 rounded-lg border border-slate-100" title="삭제"><i data-lucide="trash-2" class="w-4 h-4"></i></button>
                        </div>
                    </div>
...
            const pbt = ia ? "작동중" : "등록";
            const eb = isEditMode ? `<div class="flex gap-2 shrink-0"><button onclick="event.stopPropagation(); window.openNoticeEditModal('${pt.id}')" class="text-slate-400 hover:text-figjam p-1.5 rounded-md hover:bg-slate-100 transition-colors"><i data-lucide="edit" class="w-4 h-4"></i></button></div>` : '';
            c.insertAdjacentHTML('beforeend', `<div onclick="window.openNoticeViewModal('${pt.id}')" class="grid grid-cols-12 gap-4 p-5 border-b border-slate-50 hover:bg-slate-50/80 cursor-pointer items-center group"><div class="col-span-1 text-center text-[13px] font-bold text-slate-400">${p.length - i}</div><div class="col-span-7 md:col-span-8 flex items-center gap-3">${ia ? `<span class="bg-red-50 text-red-500 px-2 py-0.5 rounded text-[11px] font-bold shrink-0">중요</span>` : ''}<h3 class="text-[15px] font-bold text-slate-700 truncate group-hover:text-figjam transition-colors">${pt.title}</h3></div><div class="col-span-4 md:col-span-3 flex items-center justify-end gap-4 pr-2"><span class="text-[13px] text-slate-400 hidden md:block">${pt.date}</span><button onclick="event.stopPropagation(); window.promptPopupPwd('${pt.id}')" class="px-3 py-1.5 rounded-lg text-[12px] flex items-center gap-1.5 shadow-sm ${pb}"><i data-lucide="bell" class="w-3.5 h-3.5"></i> ${pbt}</button>${eb}</div></div>`);
        });
        if (isEditMode) c.insertAdjacentHTML('beforeend', `<div onclick="window.openNoticeEditModal('new')" class="p-6 flex flex-col items-center justify-center text-slate-400 hover:text-figjam bg-slate-50/50 cursor-pointer h-[120px]"><i data-lucide="plus-circle" class="w-6 h-6 mb-2"></i><span class="font-bold text-[14px]">새 작성</span></div>`);
...
        const eb = isEditMode ? `<button onclick="window.openTodayModal()" class="absolute top-4 right-4 text-slate-400 hover:text-figjam z-20 bg-slate-50 hover:bg-slate-100 p-2 rounded-xl transition-all opacity-0 group-hover/today:opacity-100"><i data-lucide="edit" class="w-4 h-4"></i></button>` : '';
        const dh = d.desc ? d.desc.split('\n').map(l => `<li>${l}</li>`).join('') : '';
        const b1Action = d.cardId ? `href="#" onclick="window.goToToolCard('${d.cardId}', '${d.bUrl1 || ''}'); return false;"` : `href="${d.bUrl1 || '#'}" ${d.bUrl1 && !d.bUrl1.startsWith('javascript:') ? 'target="_blank"' : ''}`;
...
            let eb = isEditMode ? `<div class="absolute top-3 right-3 flex items-center gap-1 opacity-0 group-hover/daily:opacity-100 z-20"><button onclick="window.toggleDailyTaskVisibility('${t.id}')" class="text-slate-400 hover:text-amber-500 bg-slate-50 p-1.5 rounded-lg shadow-sm"><i data-lucide="${t.isHidden ? 'eye-off' : 'eye'}" class="w-4 h-4"></i></button><button onclick="window.openDailyModal('${t.id}')" class="text-slate-400 hover:text-figjam bg-slate-50 p-1.5 rounded-lg shadow-sm"><i data-lucide="edit" class="w-4 h-4"></i></button></div>` : '';
...
        });
        if (isEditMode) c.innerHTML += `<div onclick="window.openDailyModal('new')" class="bg-slate-50/50 border-2 border-dashed border-slate-200 rounded-[24px] p-6 flex flex-col items-center justify-center text-slate-400 hover:text-figjam cursor-pointer min-h-[180px]"><i data-lucide="plus-circle" class="w-8 h-8 mb-2"></i><span class="font-bold text-[14px]">추가</span></div>`;
    };
...
                const ca = isEditMode ? `onclick="event.stopPropagation(); window.openScheduleModal('${ds}','${e.id}')"` : `onclick="event.stopPropagation(); window.openGeneralViewModal('event','${e.id}')"`;
...
            const dca = isEditMode ? `onclick="window.openDateEditModal('${ds}')"` : (hDi ? `onclick="window.openGeneralViewModal('date','${ds}')"` : ''); const cc = isEditMode || hDi ? 'cursor-pointer hover:bg-slate-50 hover:ring-2 hover:ring-figjam/50 hover:z-10' : '';
            g.innerHTML += `<div class="bg-white ${minH} p-2 border-t border-transparent relative flex flex-col group ${cc}" ${dca}><div class="flex items-center mb-1"><span class="text-[14px] ${it ? 'flex justify-center w-full' : 'ml-1'}"><span class="${dc} ${!it ? 'inline-block' : ''}">${i}</span></span>${!it ? dih : ''}</div>${it ? `<div class="flex justify-center w-full mb-1">${dih}</div>` : ''}${sh}${isEditMode ? `<button onclick="event.stopPropagation(); window.openScheduleModal('${ds}')" class="absolute top-2 right-2 opacity-0 group-hover:opacity-100 text-slate-400 hover:text-figjam bg-white rounded-full shadow-sm p-0.5"><i data-lucide="plus" class="w-4 h-4"></i></button>` : ''}</div>`;
        }
...
        if (currentToolLayout === 'grid') c.className = 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6';
        else c.className = 'flex flex-col gap-4';
...
            c.innerHTML += `<div onclick="window.openToolCardModal('${tId}','${lId}','new')" class="${ac}"><i data-lucide="plus-circle" class="w-8 h-8 mb-2"></i><span class="font-bold">카드 추가</span></div>`;
        }
        cds.forEach(cd => {
...
            const mb = lm.length > 0 ? `<button onclick="window.switchMemoTool('${tId}'); window.switchMemoLevel('${lId}'); window.openMemoListModal();" class="absolute -top-3 -left-3 bg-indigo-500 text-white text-[11px] font-extrabold px-2.5 py-1.5 rounded-full shadow-md z-20 flex gap-1 hover:bg-indigo-600"><i data-lucide="file-text" class="w-3.5 h-3.5"></i> ${lm.length}</button>` : '';
            const hc = cd.isHidden ? 'opacity-50 grayscale' : '';
            const eb = isEditMode ? `<div class="absolute top-3 right-3 flex gap-1 opacity-0 group-hover:opacity-100 z-20"><button onclick="event.stopPropagation(); window.promptDeleteToolCard('${tId}','${lId}','${cd.id}')" class="text-slate-400 hover:text-red-500 bg-white shadow-sm p-1.5 rounded-lg"><i data-lucide="trash-2" class="w-4 h-4"></i></button><button onclick="event.stopPropagation(); window.openToolCardModal('${tId}','${lId}','${cd.id}')" class="text-slate-400 hover:text-figjam bg-white shadow-sm p-1.5 rounded-lg"><i data-lucide="edit" class="w-4 h-4"></i></button></div>` : '';
...
        document.getElementById('tcm-links-container').insertAdjacentHTML('beforeend', `<div class="flex gap-2 tcm-link-item bg-white p-2 rounded-lg border border-slate-100"><div class="flex-1 flex flex-col gap-2"><input type="text" placeholder="텍스트" value="${t}" class="link-text w-full border-2 border-slate-100 rounded-md p-2 text-xs font-bold outline-none bg-slate-50"><input type="text" placeholder="URL" value="${u}" class="link-url w-full border-2 border-slate-100 rounded-md p-2 text-xs outline-none bg-slate-50"></div><button type="button" onclick="this.parentElement.remove()" class="text-slate-300 hover:text-red-500 p-2 shrink-0 bg-slate-50"><i data-lucide="trash-2" class="w-4 h-4"></i></button></div>`); lucide.createIcons();
    };

    window.addToolButtonRow = function(t = '', u = '') {
        document.getElementById('tcm-buttons-container').insertAdjacentHTML('beforeend', `<div class="flex gap-2 tcm-button-item bg-white p-2 rounded-lg border border-slate-100"><div class="flex-1 flex flex-col gap-2"><input type="text" placeholder="버튼 텍스트" value="${t}" class="btn-text w-full border-2 border-slate-100 rounded-md p-2 text-xs font-bold outline-none bg-slate-50"><input type="text" placeholder="URL" value="${u}" class="btn-url w-full border-2 border-slate-100 rounded-md p-2 text-xs outline-none bg-slate-50"></div><button type="button" onclick="this.parentElement.remove()" class="text-slate-300 hover:text-red-500 p-2 shrink-0 bg-slate-50"><i data-lucide="trash-2" class="w-4 h-4"></i></button></div>`); lucide.createIcons();
    };
...
        if (isEditMode) c.innerHTML += `<div onclick="window.openWorkspaceModal('new')" class="bg-slate-50/50 border-2 border-dashed border-slate-200 rounded-[24px] p-6 flex flex-col items-center justify-center text-slate-400 hover:text-figjam cursor-pointer min-h-[250px]"><i data-lucide="plus-circle" class="w-10 h-10 mb-3"></i><span class="font-bold text-[15px]">추가</span></div>`;
        workspaceData.forEach(u => {
            let eb = isEditMode ? `<button onclick="window.openWorkspaceModal('${u.id}')" class="absolute top-4 right-4 text-slate-400 hover:text-figjam bg-slate-50 p-2 rounded-xl opacity-0 group-hover:opacity-100 z-20"><i data-lucide="edit" class="w-4 h-4"></i></button>` : '';
            let lh = '';
            (u.links || []).forEach(l => {
                if (!l.name) return;
...
                    </a>
                    <button onclick="window.copyToClipboard('${l.url}')" class="w-[46px] h-[46px] flex items-center justify-center bg-slate-50 border border-slate-100 rounded-xl text-slate-400 hover:text-figjam hover:bg-slate-100 transition-colors shrink-0 shadow-sm" title="링크 복사"><i data-lucide="copy" class="w-4 h-4"></i></button>
                </div>`;
            });
...
                            <button onclick="window.copyToClipboard('${u.email || 'abcd123@gmail.com'}')" class="ml-1 text-slate-300 hover:text-figjam transition-colors shrink-0" title="이메일 복사"><i data-lucide="copy" class="w-3 h-3"></i></button>
                        </div>
                    </div>
                </div>
...
        (u.links || []).forEach(l => {
            c.insertAdjacentHTML('beforeend', `<div class="wsm-link-item bg-slate-50 p-4 rounded-xl relative group"><button type="button" onclick="this.parentElement.remove()" class="absolute top-2 right-2 text-slate-300 hover:text-red-500 bg-white rounded-md p-1"><i data-lucide="trash-2" class="w-3 h-3"></i></button><div class="grid grid-cols-2 gap-3 mb-3"><div><input type="text" value="${l.name}" class="w-full border-2 border-slate-200 rounded-lg p-2 text-xs font-bold bg-white link-name"></div><div><input type="text" value="${l.icon || 'link'}" class="w-full border-2 border-slate-200 rounded-lg p-2 text-xs bg-white link-icon"></div></div><input type="text" value="${l.url}" class="w-full border-2 border-slate-200 rounded-lg p-2 text-xs bg-white link-url"></div>`);
        });
...
            const eb = isEditMode ? `<button onclick="event.stopPropagation(); window.openMemoEditModal('${m.id}')" class="text-slate-400 hover:text-indigo-600 p-1.5 rounded-md hover:bg-indigo-50"><i data-lucide="edit" class="w-4 h-4"></i></button>` : '';
            c.insertAdjacentHTML('beforeend', `<div onclick="window.openMemoViewModal('${m.id}')" class="grid grid-cols-12 gap-4 p-4 border-b border-slate-50 hover:bg-slate-50/80 cursor-pointer items-center group"><div class="col-span-1 text-center text-[13px] font-bold text-slate-400">${fm.length - i}</div><div class="col-span-3 truncate text-[12px] font-medium text-slate-400 bg-slate-50 px-2 py-1 rounded-md" title="${ct}">${ct}</div><div class="col-span-5 md:col-span-6 flex items-center gap-3"><h3 class="text-[14px] font-bold text-slate-700 truncate group-hover:text-indigo-600">${m.title}</h3></div><div class="col-span-3 md:col-span-2 flex justify-end gap-3 pr-2"><span class="text-[12px] font-medium text-slate-400">${m.date}</span>${eb}</div></div>`);
        });
        if (isEditMode) {
            c.insertAdjacentHTML('beforeend', `<div onclick="window.openMemoEditModal('new')" class="p-6 flex flex-col items-center justify-center text-slate-400 hover:text-indigo-600 bg-slate-50/50 cursor-pointer h-[100px]"><i data-lucide="plus-circle" class="w-5 h-5 mb-1.5"></i><span class="font-bold text-[13px]">새 작성</span></div>`);
        } else if (fm.length === 0) {
...
            btn.onclick = () => { window.toggleDataset(i); window.updateLegendStyle(i); }; btn.id = `legend-btn-${i}`;
            const cb = isEditMode ? `<button onclick="event.stopPropagation(); window.openCommentModal('${s}')" class="w-7 h-7 flex items-center justify-center bg-indigo-50 hover:bg-indigo-500 hover:text-white rounded-lg text-indigo-400 ml-1"><i data-lucide="message-square" class="w-3.5 h-3.5"></i></button>` : '';
            btn.innerHTML = `<div class="flex items-center gap-3"><div class="w-3 h-3 rounded-full" style="background-color: ${studentColors[i % studentColors.length]}"></div><span class="text-[14px] font-bold text-slate-700">${s}</span></div><div class="flex items-center gap-1.5"><span class="text-[12px] font-bold text-slate-400 bg-slate-100 px-2 py-0.5 rounded-md mr-1">${av}%</span><span onclick="event.stopPropagation(); window.openModal('${s}')" class="w-7 h-7 flex items-center justify-center bg-slate-100 hover:bg-figjam hover:text-white rounded-lg text-slate-400"><i data-lucide="edit-2" class="w-3.5 h-3.5"></i></span>${cb}</div>`;
...
            const r = doc(db, 'uiux_dashboard', 'shared_state');
            try {
                await setDoc(r, {
                    menuOrder, notice: noticeData, memo: memoData, designFilters, progress: progressData, tool: toolData, roadmap: roadmapData, workspace: workspaceData, studentComments, schedule: scheduleData, dateLink: dateData, board: boardData, lastUpdated: Date.now()
                });
            } catch (e) {
                console.error("클라우드 저장 실패", e);
            }
        } else {
            localStorage.setItem('menuOrder_v1', JSON.stringify(menuOrder));
            localStorage.setItem('noticeData_v1', JSON.stringify(noticeData));
            localStorage.setItem('memoData_v1', JSON.stringify(memoData));
            localStorage.setItem('designFilters_v1', JSON.stringify(designFilters));
            localStorage.setItem('roadmapData_v1', JSON.stringify(roadmapData));
            localStorage.setItem('toolCardsData_v19', JSON.stringify(toolData));
            localStorage.setItem('workspaceData_v3', JSON.stringify(workspaceData));
            localStorage.setItem('assignmentData_v2', JSON.stringify(progressData));
            localStorage.setItem('studentComments_v1', JSON.stringify(studentComments));
            localStorage.setItem('scheduleData_v1', JSON.stringify(scheduleData));
            localStorage.setItem('dateLink_v1', JSON.stringify(dateData));
            localStorage.setItem('boardData_v1', JSON.stringify(boardData));
        }
    };
...
        const mo = localStorage.getItem('menuOrder_v1') || localStorage.getItem('menuOrder'); if (mo) menuOrder = JSON.parse(mo);
        const nt = localStorage.getItem('noticeData_v1') || localStorage.getItem('noticeData'); if (nt) noticeData = JSON.parse(nt);
        const md = localStorage.getItem('memoData_v1') || localStorage.getItem('memoData'); if (md) memoData = JSON.parse(md);
        const df = localStorage.getItem('designFilters_v1'); if (df) designFilters = JSON.parse(df);
        const rm = localStorage.getItem('roadmapData_v1') || localStorage.getItem('roadmapData'); if (rm) roadmapData = JSON.parse(rm);
        const tl = localStorage.getItem('toolCardsData_v19'); if (tl) toolData = JSON.parse(tl);
        const ws = localStorage.getItem('workspaceData_v3'); if (ws) workspaceData = JSON.parse(ws);
        const pg = localStorage.getItem('assignmentData_v2'); if (pg) progressData = JSON.parse(pg);
        const sc = localStorage.getItem('studentComments_v1'); if (sc) studentComments = JSON.parse(sc);
        const sd = localStorage.getItem('scheduleData_v1'); if (sd) scheduleData = JSON.parse(sd);
        const dd = localStorage.getItem('dateLink_v1'); if (dd) dateData = JSON.parse(dd);
        const bd = localStorage.getItem('boardData_v1'); if (bd) boardData = JSON.parse(bd);
        window.forceDataSync(); window.updateDesignFilterSelects(); window.renderAll();
    };

    window.initFirebaseAndLoad = function() {
        if (auth && db) {
...
                        if (s.exists()) {
                            const d = s.data();
                            if (d.menuOrder) menuOrder = d.menuOrder;
                            if (d.notice) noticeData = d.notice;
                            if (d.memo) memoData = d.memo;
                            if (d.designFilters) designFilters = d.designFilters;
                            if (d.progress) progressData = d.progress;
                            if (d.tool) toolData = d.tool;
                            if (d.roadmap) roadmapData = d.roadmap;
                            if (d.workspace) workspaceData = d.workspace;
                            if (d.studentComments) studentComments = d.studentComments;
                            if (d.schedule) scheduleData = d.schedule;
                            if (d.dateLink) dateData = d.dateLink;
                            if (d.board) boardData = d.board;
                            
                            const cs = document.getElementById('cloud-status');
                            if (cs) {
                                cs.textContent = "클라우드 연동됨";
                                cs.classList.remove('opacity-0', 'text-slate-400');
                                cs.classList.add('opacity-100', 'text-emerald-500');
                            }
...
        const state = { menuOrder, noticeData, memoData, designFilters, progressData, toolData, roadmapData, workspaceData, studentComments, scheduleData, dateData, boardData };
        document.getElementById('backup-textarea').value = JSON.stringify(state);
...
            if (st.menuOrder) menuOrder = st.menuOrder;
            if (st.noticeData) noticeData = st.noticeData;
            if (st.memoData) memoData = st.memoData;
            if (st.designFilters) designFilters = st.designFilters;
            if (st.progressData) progressData = st.progressData;
            if (st.toolData) toolData = st.toolData;
            if (st.roadmapData) roadmapData = st.roadmapData;
            if (st.workspaceData) workspaceData = st.workspaceData;
            if (st.studentComments) studentComments = st.studentComments;
            if (st.scheduleData) scheduleData = st.scheduleData;
            if (st.dateData) dateData = st.dateData;
            if (st.boardData) boardData = st.boardData;
