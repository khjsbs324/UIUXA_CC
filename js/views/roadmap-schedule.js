(function() {
    ...
    function findToolCard(toolData, cardId) {
        let found = null;
        toolIds.forEach((toolId) => {
            levelIds.forEach((levelId) => {
                const card = (toolData[toolId][levelId] || []).find((item) => item.id === cardId);
                if (card) found = { card, toolId, levelId };
            });
        });
        return found;
    }

    function fillLinkedSelects(noticeData, toolData, noticeSelect, toolSelect, selected = {}) {
        ...
        toolIds.forEach((toolId) => {
            levelIds.forEach((levelId) => {
                (toolData[toolId][levelId] || []).forEach((card) => {
                    toolSelect.innerHTML += `<option value="${card.id}">[${toolId}] ${card.title}</option>`;
                });
            });
        });
    ...
    function installRoadmapScheduleView(context) {
        ...
        const renderCalGrid = function(calendarDate, gridId, captionId, minHeightClass) {
            ...
            for (let day = 1; day <= daysInMonth; day++) {
                ...
                if (dateInfo.memoContent) dateIcons += '<i data-lucide="file-text" class="w-3 h-3 text-indigo-500"></i>';
                const dateIconHtml = dateIcons ? `<span class="flex gap-1 ml-1 bg-slate-100 px-1 rounded">${dateIcons}</span>` : '';
                const hasDateInfo = Object.keys(dateInfo).length > 0;
                ...
                grid.innerHTML += `<div class="bg-white ${minHeightClass} p-2 border-t border-transparent relative flex flex-col group ${cellClass}" ${dateAction}><div class="flex items-center mb-1"><span class="text-[14px] ${isToday ? 'flex justify-center w-full' : 'ml-1'}"><span class="${dayClass} ${!isToday ? 'inline-block' : ''}">${day}</span></span>${!isToday ? dateIconHtml : ''}</div>${isToday ? `<div class="flex justify-center w-full mb-1">${dateIconHtml}</div>` : ''}${scheduleHtml}${isEditMode ? `<button ${actionAttrs('openScheduleModal', [dateString], { stop: true })} class="absolute top-2 right-2 opacity-0 group-hover:opacity-100 text-slate-400 hover:text-figjam bg-white rounded-full shadow-sm p-0.5"><i data-lucide="plus" class="w-4 h-4"></i></button>` : ''}</div>`;
            }
        ...
        window.renderTodayTask = function() {
            ...
            const primaryAction = todayTask.cardId ? `href="#" ${actionAttrs('goToToolCard', [todayTask.cardId, todayTask.bUrl1 || ''], { prevent: true })}` : `href="${todayTask.bUrl1 || '#'}" ${todayTask.bUrl1 && !todayTask.bUrl1.startsWith('javascript:') ? 'target="_blank"' : ''}`;
        ...
        window.openTodayModal = function() {
            ...
            document.getElementById('ttm-bUrl1').value = todayTask.bUrl1 || '';
                ...
                toolIds.forEach((toolId) => {
                    levelIds.forEach((levelId) => {
                        (getToolData()[toolId][levelId] || []).forEach((card) => {
                            ...
                            cardSelect.innerHTML += `<option value="${card.id}" ${selected}>[${toolId}] ${card.title}</option>`;
                        });
                    });
                });
            }
        ...
        window.saveTodayModal = function() {
            ...
            roadmapData.todayTask = {
                ...
                bUrl1: document.getElementById('ttm-bUrl1').value.trim(),
        ...
        window.openScheduleModal = function(dateString, id = null) {
            ...
            fillLinkedSelects(getNoticeData(), getToolData(), noticeSelect, toolSelect);
            if (id) {
        ...
        window.openDateEditModal = function(dateString) {
            ...
            fillLinkedSelects(getNoticeData(), getToolData(), document.getElementById('dem-noticeId'), document.getElementById('dem-toolCardId'), dateInfo);
        ...
        window.saveDateEditModal = function() {
            ...
            const dateData = getDateData();
            if (!noticeId && !toolCardId && !memoContent) delete dateData[dateString];
            else dateData[dateString] = { noticeId, toolCardId, memoContent };
            saveToCloud();
            window.renderFullCalendar();
            window.renderCalendar();
        ...
        window.openGeneralViewModal = function(type, payload) {
            ...
            if (type === 'date') {
                ...
                title = '날짜 연결 정보';
                dateText = payload.replace(/-/g, '. ');
            } else {
                target = scheduleData.find((event) => event.id === payload) || {};
                title = target.title;
                dateText = target.date.replace(/-/g, '. ');
            }
            ...
            if (target.memoContent) {
                hasContent = true;
                links.innerHTML += `<div class="p-4 bg-indigo-50/50 border border-indigo-100 rounded-xl mb-3"><div class="flex gap-2 mb-2 text-indigo-600"><i data-lucide="file-text" class="w-4 h-4"></i><span class="text-xs font-bold">작성 메모</span></div><div class="text-[13px] text-slate-700 whitespace-pre-wrap">${target.memoContent}</div></div>`;
            }
            if (target.noticeId) {
            ...
            if (target.toolCardId) {
                const match = findToolCard(toolData, target.toolCardId);
                if (match) {
                    hasContent = true;
