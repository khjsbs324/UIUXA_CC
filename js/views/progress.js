(function() {
    ...
    function installProgressView(context) {
        ...
        window.updateSummary = function() {
            ...
            let total = 0;
            let count = 0;
            Object.values(progressData).forEach((studentProgress) => {
                Object.values(studentProgress).forEach((value) => {
        ...
        window.openModal = function(student) {
            ...
            grid.innerHTML = tasksList.map((task, index) => `
                <div class="relative bg-slate-50 p-4 rounded-xl border border-slate-100">
                    <label class="block text-[11px] font-bold text-slate-500 mb-2 truncate"><span class="text-slate-300 mr-1">${index + 1}.</span>${task}</label>
                    <div class="relative">
